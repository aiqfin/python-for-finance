...
\begin{frame}[fragile]{read\_csv：读入真实数据}
	\begin{lstlisting}
df = pd.read_csv("eod_data.csv",       # 文件路径
                 index_col=0,          # 第0列作为索引
...
\end{lstlisting}
	\begin{itemize}
		...
		\item 导出同样简单：\texttt{df.to\_csv("result.csv")}。
		\item Excel：\texttt{pd.read\_excel("data.xlsx")}（需安装 openpyxl）。
	\end{itemize}
	\begin{alertblock}{路径小贴士}
		文件与 notebook 放同一文件夹时直接写文件名即可；\\
		路径中尽量避免中文与空格。
	\end{alertblock}
\end{frame}
