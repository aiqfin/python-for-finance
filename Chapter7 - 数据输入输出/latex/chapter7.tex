...
# 读文件：逐行看回来
with open("../outputs/notes.txt", encoding="utf-8") as f:
    for line in f:
        print(line.strip())
\end{lstlisting}
	\begin{itemize}
...
df = pd.read_csv("../data/eod_data.csv",
                 index_col=0,       # 第0列作索引
...
stats.to_csv("../outputs/stats.csv")            # 默认带索引列
stats.to_csv("../outputs/stats_no_index.csv",
             index=False)            # 不写索引
\end{lstlisting}
	\begin{itemize}
		\item \texttt{to\_csv} 与 \texttt{read\_csv} 是一对"镜像"操作。
		\item 下次打开 notebook，直接 \texttt{read\_csv("../outputs/stats.csv")}——不用重算。
	\end{itemize}
...
\begin{frame}[fragile]{读回验证}
	\begin{lstlisting}
back = pd.read_csv("../outputs/stats.csv", index_col=0)
back.head(3)
...
df.to_excel("../outputs/data.xlsx",            # 写
            sheet_name="价格")
stats.to_excel("../outputs/data.xlsx",
               sheet_name="统计")   # 第二个表

pd.read_excel("../outputs/data.xlsx",          # 读
              sheet_name="统计",
              index_col=0)
\end{lstlisting}
	\begin{itemize}
...
df.to_hdf("../outputs/data.h5", key="prices",
          mode="w", complevel=9, complib="zlib")

# 读：一行读回完整DataFrame，日期索引原样保留
back = pd.read_hdf("../outputs/data.h5", "prices")
...
\begin{frame}[fragile]{综合案例：一条完整的分析流水线}
	\begin{lstlisting}
# 1. 读入原始数据（第6章已熟悉）
df = pd.read_csv("../data/eod_data.csv", index_col=0,
...
# 3. 保存中间结果：明天不用重算
vol.dropna().to_csv("../outputs/spy_vol21.csv", header=True)

# 4. 保存给报告的Excel版本
vol.dropna().to_excel("../outputs/spy_vol21.xlsx")
\end{lstlisting}
	\begin{itemize}
